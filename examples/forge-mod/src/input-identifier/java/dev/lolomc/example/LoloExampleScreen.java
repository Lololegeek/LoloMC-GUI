...
final class LoloExampleScreen extends Screen {
    ...
    static LoloExampleScreen create(Minecraft client) {
        try (InputStream xml = resource(client, LoloGuiForgeExample.resource("ui/screen.xml"));
            ...
            GuiSession session = LoloGui.session(document).on("play", node -> { }).on("close", node -> client.setScreen(null));
            return new LoloExampleScreen(session);
        } catch (IOException error) { throw new IllegalStateException("Unable to load LoloMC GUI example resources", error); }
    }
    private static InputStream resource(Minecraft client, Identifier id) throws IOException {
