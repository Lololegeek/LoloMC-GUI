...
| Minecraft 1.12.2 to 1.21.x | Core architecture-compatible | version-specific rendering calls are isolated in `RenderBackend`; see the exact example-build matrix |
| Minecraft 26.1+ | Separate adapter required | unobfuscated code, Java 25, and the extracted GUI rendering pipeline are breaking changes |
...
- 1.20–1.21.x: `GuiGraphics` (Mojang mappings) or `DrawContext` (Yarn) backend.
- 26.1+: `GuiGraphicsExtractor` and `extractRenderState` backend.
